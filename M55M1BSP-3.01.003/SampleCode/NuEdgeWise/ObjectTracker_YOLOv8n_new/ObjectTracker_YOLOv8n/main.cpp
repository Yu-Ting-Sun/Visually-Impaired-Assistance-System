...
int main()
{
    ...
    /* UART bridge smoke test before SD/model/camera init.
       Expected ESP32 raw bytes are AA 01 FF 01 FF 55, repeated every second. */
    VoicePlay_UART_Init();  /* UART2: D1/PB1(TX) -> ESP32 RX/GPIO3 at 115200 baud */
    VoiceWarning_t test_warn;
    test_warn.direction = 1;   /* CENTER */
    test_warn.class_id = 0xFF; /* UNKNOWN */
    test_warn.severity = 1;    /* CAUTION */

    while (1) {
        VoicePlay_SendWarning(&test_warn);
